...
import {
  ...
  Search, Plus, Filter, Sparkles, AlertTriangle, RefreshCw, X, CheckCircle2
} from 'lucide-react';
import { fetchVademecum, createVademecumItem, deleteVademecumItem } from '../api';
import SejourCutoffMapModal from './sejour/SejourCutoffMapModal';
import SejourDepartureChecklistModal from './sejour/SejourDepartureChecklistModal';
import SejourTaskModal from './sejour/SejourTaskModal';
import SejourEditModal from './sejour/SejourEditModal';
...
export default function VademecumPage({ properties, currentUser }) {
  ...
  // Triptyque Confort Énergétique State
  // 1. Chauffage
  const [heatingTemp, setHeatingTemp] = useState(19.5);
  const [heatingScheduleStart, setHeatingScheduleStart] = useState('Ven. 19 oct. — 14:00');
  const [heatingScheduleStop, setHeatingScheduleStop] = useState('Dim. 25 oct. — 18:30');
  const [heatingSyncing, setHeatingSyncing] = useState(false);

  // 2. Eau Chaude
  const [dhwTemp, setDhwTemp] = useState(55.0);
  const [dhwScheduleStart, setDhwScheduleStart] = useState('Ven. 19 oct. — 12:00');
  const [dhwScheduleStop, setDhwScheduleStop] = useState('Dim. 25 oct. — 19:00');

  // 3. Piscine
  const [poolTemp, setPoolTemp] = useState(14.0);

  const handleHeatingMinus = () => {
    if (heatingTemp <= 15.0) return;
    const nextVal = Math.round((heatingTemp - 0.5) * 10) / 10;
    setHeatingTemp(nextVal);
    triggerHeatingSync(nextVal);
  };

  const handleHeatingPlus = () => {
    if (heatingTemp >= 20.0) {
      alert('Consigne maximale autorisée par la charte des associés : 20.0°C.');
      return;
    }
    const nextVal = Math.round((heatingTemp + 0.5) * 10) / 10;
    setHeatingTemp(nextVal);
  ...
  const triggerHeatingSync = (val) => {
    setHeatingSyncing(true);
    setTimeout(() => {
      setHeatingSyncing(false);
      showToast(`Consigne PAC Viessmann mise à jour : ${val.toFixed(1)}°C`);
    }, 800);
  };

  const handleDhwMinus = () => {
    if (dhwTemp <= 45.0) return;
    const nextVal = Math.round((dhwTemp - 0.5) * 10) / 10;
    setDhwTemp(nextVal);
    showToast(`Consigne Eau Chaude : ${nextVal.toFixed(1)}°C`);
  };

  const handleDhwPlus = () => {
    if (dhwTemp >= 60.0) return;
    const nextVal = Math.round((dhwTemp + 0.5) * 10) / 10;
    setDhwTemp(nextVal);
    showToast(`Consigne Eau Chaude : ${nextVal.toFixed(1)}°C`);
  };

  const handlePoolMinus = () => {
    if (poolTemp <= 10.0) return;
    const nextVal = Math.round((poolTemp - 0.5) * 10) / 10;
    setPoolTemp(nextVal);
    showToast(`Consigne Piscine : ${nextVal.toFixed(1)}°C`);
  };

  const handlePoolPlus = () => {
    if (poolTemp >= 28.0) return;
    const nextVal = Math.round((poolTemp + 0.5) * 10) / 10;
    setPoolTemp(nextVal);
    showToast(`Consigne Piscine : ${nextVal.toFixed(1)}°C`);
  };
  ...
  return (
    <div className="relative w-full max-w-[1360px] mx-auto pb-16 selection:bg-emerald-100 selection:text-emerald-900">
          ...
          <div className="flex flex-col gap-4 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-1">
              ...
              <div className="inline-flex items-center gap-2 p-1.5 px-3 rounded-xl bg-canvas-slate border border-border-subtle text-on-surface shadow-sm">
                ...
                <div className="flex items-center gap-1.5 font-label-sm text-label-sm">
                  <span className="font-semibold text-on-surface">Sophie Jamet</span>
                </div>
              </div>
            </div>
      ...
      {/* ===================================================================== */}
      {/* 3. TRIPTYQUE : RÉGULATION & CONFORT ÉNERGÉTIQUE                      */}
      {/* ===================================================================== */}
          ...
          <div className="p-5 rounded-2xl bg-canvas-slate border border-border-subtle flex flex-col justify-between gap-5 shadow-sm">
            <div className="flex flex-col gap-4">
              ...
              <div className="space-y-2.5">
                <div className="p-2.5 rounded-xl bg-white border border-border-subtle flex items-center justify-between gap-2 shadow-sm">
                  ...
                  <button
                    onClick={() => {
                      const newTime = prompt('Modifier mise en marche chauffage :', heatingScheduleStart);
                      if (newTime) setHeatingScheduleStart(newTime);
                    }}
                ...
                <div className="p-2.5 rounded-xl bg-white border border-border-subtle flex items-center justify-between gap-2 shadow-sm">
                  ...
                  <button
                    onClick={() => {
                      const newTime = prompt('Modifier arrêt chauffage :', heatingScheduleStop);
                      if (newTime) setHeatingScheduleStop(newTime);
                    }}
                    className="h-8 px-3 rounded-lg bg-surface-container hover:bg-surface-container-high text-primary font-label-sm text-xs font-semibold shrink-0 transition-colors"
                    type="button"
                  >
            ...
            <div className="pt-2 border-t border-border-subtle flex items-center justify-between text-xs text-on-surface-variant">
              <span className="flex items-center gap-1">
                ...
                PAC Viessmann Vitocal
              </span>
              <span className="font-medium text-primary">
                {heatingSyncing ? 'Synchronisation...' : 'Mode Confort auto'}
              </span>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="space-y-2.5">
                <div className="p-2.5 rounded-xl bg-white border border-border-subtle flex items-center justify-between gap-2 shadow-sm">
                  ...
                  <button
                    onClick={() => {
                      const newTime = prompt('Modifier mise en marche eau chaude :', dhwScheduleStart);
                      if (newTime) setDhwScheduleStart(newTime);
                    }}
                    className="h-8 px-3 rounded-lg bg-surface-container hover:bg-surface-container-high text-primary font-label-sm text-xs font-semibold shrink-0 transition-colors"
                    type="button"
                  >
                ...
                <div className="p-2.5 rounded-xl bg-white border border-border-subtle flex items-center justify-between gap-2 shadow-sm">
                  ...
                  <button
                    onClick={() => {
                      const newTime = prompt('Modifier arrêt eau chaude :', dhwScheduleStop);
                      if (newTime) setDhwScheduleStop(newTime);
                    }}
                    className="h-8 px-3 rounded-lg bg-surface-container hover:bg-surface-container-high text-primary font-label-sm text-xs font-semibold shrink-0 transition-colors"
                    type="button"
                  >
